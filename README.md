...
- **校舎管理**: 定例議題を校舎ごとに分けたい場合の校舎の登録・削除。トップページ上部のドロップダウンで校舎を切り替えると、その校舎の定例議題（目標・現状・体験数・具体的行動・内部充実のアクション）が表示される（議題・課題・個人タスクは全校舎共通）
- 各カードは展開すると詳細説明・事前コメント（メモ）のやり取りができ、「編集」ボタンからタイトル・詳細・種類・担当者・期限を後から修正できます
...
1. 「メンバー管理」でスタッフを登録（校舎ごとに定例議題を分けたい場合は「校舎管理」で校舎も登録）
2. 会議の準備期間中、議題や課題を「今後のストック」または「今週の会議」に追加し、担当者・期限を設定（後から各カードの「編集」で修正可）
